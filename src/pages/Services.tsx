...
import { FaSearch, FaMapMarkerAlt, FaMoneyBillWave, FaFilter, FaSpinner } from 'react-icons/fa';
    ...
    <div className="bg-gradient-dark min-h-screen text-white">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-16 text-center">
          <h1 className="text-5xl md:text-6xl font-bold mb-4">
            <span className=" bg-clip-text text-cyan-600 ">{t('services.discoverServices')}</span>
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-12">
        ...
        <div className="mb-8">
          ...
          <div className={`bg-card-dark rounded-2xl p-6 shadow-2xl transition-all duration-300 ${showFilters || 'max-md:hidden'}`}>
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-400 font-medium">{t('services.to')}</span>
                  <div className="flex-1">
                    <input
                      type="number"
                      value={maxPrice}
                ...
                <div className="text-xs text-gray-400 mt-1">
                  {t('services.priceRangeDisplay', { min: minPrice, max: maxPrice })}
                </div>
              </div>
            </div>
          </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-300 text-lg">{t('services.loadingServices')}</p>
          </div>
        ) : (
          <>
            {/* Results Header */}
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-white">
                {filteredServices.length > 0 ? (
                  <>
                    {t('services.showingResults', { count: filteredServices.length, plural: filteredServices.length !== 1 ? 's' : '' })}
                    {(searchTerm || selectedCity) && (
                      <span className="text-gray-400 text-base font-normal ml-2">
                        {searchTerm && t('services.searchFor', { term: searchTerm })}
                        {searchTerm && selectedCity && ' '}
                        {selectedCity && t('services.inCity', { city: selectedCity })}
                      </span>
                    )}
                  </>
                ) : (
            ...
            <div className="space-y-6">
              {filteredServices.length > 0 ? (
                ...
                <div className="text-center py-20">
                  ...
                  <h3 className="text-2xl font-semibold text-white mb-2">{t('services.noServicesFoundTitle')}</h3>
